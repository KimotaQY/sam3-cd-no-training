...
def binary_accuracy_sklearn(pred, label):
    """
    使用sklearn计算二分类评估指标
    """
    # 确保输入是1维数组
...
def binary_accuracy_torchmetrics(pred, label):
    ...
    # 转为二值
    pred_binary = (pred >= 0.5).float().int()
    label_binary = (label >= 0.5).float().int()
    ...
    iou = JaccardIndex(task="binary", zero_division=0)

    # 计算指标
    acc = accuracy(pred_binary, label_binary)
    prec = precision(pred_binary, label_binary)
    rec = recall(pred_binary, label_binary)
...
def CaclTP(imPred, imLab, numClass):
    ...
    (TP_hist, _) = np.histogram(TP, bins=numClass, range=(1, numClass + 1))
    # print(TP.shape)
    # print(TP_hist)
